        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-6 text-sm">
              <Link href="/about" className="text-gray-600 hover:text-green-500 transition-colors">About Us</Link>
              <Link href="/account" className="text-gray-600 hover:text-green-500 transition-colors">My Account</Link>
              <Link href="/wishlist" className="text-gray-600 hover:text-green-500 transition-colors">Wishlist</Link>
              <Link href="/order-tracking" className="text-gray-600 hover:text-green-500 transition-colors">Order Tracking</Link>
            </div>
            ...
            <div className="flex items-center space-x-6 text-sm">
              <span className="text-gray-600">
                Need help? Call Us: <strong className="text-green-500">+ 1800 900</strong>
              </span>
              
              {/* Language Dropdown */}
              <div className="relative group">
                <button className="flex items-center text-gray-600 hover:text-green-500" aria-label="Select language">
                  English
                ...
                <div className="absolute right-0 mt-2 w-32 bg-white rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                  <Link href="#" className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                    <Image src="/flags/flag-fr.png" alt="French" width={16} height={12} className="mr-2" />
                    Français
                  </Link>
                  <Link href="#" className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                    <Image src="/flags/flag-dt.png" alt="German" width={16} height={12} className="mr-2" />
                    Deutsch
                  </Link>
                </div>
              </div>
              ...
              {/* Currency Dropdown */}
              <div className="relative group">
                <button className="flex items-center text-gray-600 hover:text-green-500" aria-label="Select currency">
                  USD
                  <svg className="w-4 h-4 ml-1" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
                ...
                <div className="absolute right-0 mt-2 w-24 bg-white rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                  <Link href="#" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">INR</Link>
                  <Link href="#" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">EUR</Link>
                  <Link href="#" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">GBP</Link>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex-1 max-w-2xl mx-8">
              <div className="flex">
                ...
                <div className="relative">
                  {/* Shadcn UI Combobox for categories with search */}
                  {/* Assuming you have installed @/components/ui/combobox or similar from shadcn */}
                  {/* Otherwise, you may need to copy the Combobox component structure from shadcn/ui documentation */}
                  {/* Also assumes you have the necessary state, which we add inline here */}
                  {/* Move all this to top-level of the component if used in a loop! */}
                  {(() => {
                    const categories = [
                      "All Categories",
                      "Milks and Dairies",
                    ...
                    // React hooks must be used at the component top-level:
                    // --- move these outside of this IIFE and at top of your Header (or relevant) component!
                    // const [selectedCategory, setSelectedCategory] = React.useState(categories[0]);
                    // const [search, setSearch] = React.useState("");

                    // Here written as if the code can be moved up:
                    ...
                    return (
                      <div className="w-48">
                        <Combobox
                          ...
                          inputProps={{
                            className:
                              "bg-gray-100 border-r border-gray-300 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 rounded-l-lg",
                            "aria-label": "Select product category",
                          }}
                          inputClassName="bg-gray-100 rounded-l-lg"
                          menuClassName="mt-2 z-50"
                          filterable
                        />
                      </div>
                    );
                  <select className="bg-gray-100 border-r border-gray-300 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 rounded-l-lg" aria-label="Select product category">
                ...
                <input
                  ...
                  className="flex-1 px-4 py-3 border-t border-b border-gray-300 focus:outline-none focus:ring-2 focus:ring-green-500"
                />
                
                {/* Search Button */}
                <button className="bg-green-500 text-white px-6 py-3 rounded-r-lg hover:bg-green-600 transition-colors" aria-label="Search products">
              ...
              <div className="flex flex-col items-center relative">
                <div className="relative">
                  ...
                  >
                    ...
                    <span className="absolute -top-2 -right-2 bg-blue-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">2</span>
                  </button>
                </div>
                    ...
                    <div className="p-4">
                      {cartItems.map((item) => (
                        <div key={item.id} className="flex items-center space-x-3 py-3 border-b last:border-b-0">
                          ...
                          <button className="text-red-500 hover:text-red-700" aria-label="Remove item from cart">
                      ...
                      <div className="pt-4">
                        <div className="flex justify-between items-center mb-4">
                          <span className="font-semibold">Total: ${cartItems.reduce((sum, item) => sum + (item.price * item.quantity), 0)}</span>
                        </div>
                        <div className="flex space-x-2">
                          <Link href="/cart" className="flex-1 bg-gray-100 text-center py-2 rounded hover:bg-gray-200 transition-colors">
                            View Cart
                          </Link>
                          <Link href="/checkout" className="flex-1 bg-green-500 text-white text-center py-2 rounded hover:bg-green-600 transition-colors">
                            Checkout
                          </Link>
                        </div>
                      </div>

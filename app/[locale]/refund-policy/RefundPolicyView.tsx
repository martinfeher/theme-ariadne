...
import {
  ...
  REFUND_POLICY_SECTIONS_WITH_LIST,
      ...
      <main className="container mx-auto px-4 py-8 lg:py-12">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_240px] lg:items-start">
          <article className="min-w-0 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm sm:p-8 lg:p-10">
            {REFUND_POLICY_SECTIONS.map((sectionId, index) => (
              <section
                key={sectionId}
                id={sectionId}
              ...
              >
                ...
                {REFUND_POLICY_SECTIONS_WITH_LIST.includes(sectionId) && (
          ...
          <aside className="space-y-4 lg:sticky lg:top-24">
            <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
              ...
              <nav className="mt-3" aria-label={t('tocTitle')}>
                <ul className="space-y-1.5">
                  {REFUND_POLICY_SECTIONS.map((sectionId) => (
                    <li key={sectionId}>
                      <a
                        href={`#${sectionId}`}
            ...
            <div className="rounded-xl border border-green-100 bg-green-50 p-5">
              <h2 className="text-sm font-bold text-gray-900">{t('helpTitle')}</h2>
              <p className="mt-2 text-sm text-gray-600">{t('helpDesc')}</p>
